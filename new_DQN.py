...
class DQN:
    ...
    def replay(self):
        ...
        for state, action, reward, sprime, done in batch:
            ...
            print("First prediction (target+state): ", prediction)
            print("First prediction (model+state): ", self.model.predict(state)[0])
            ...
            # proxy (=bootstrapping)
                ...
                print("Second prediction (target+sprime): ", self.target.predict(sprime)[0])
                print("Second prediction (model+sprime): ", self.model.predict(sprime)[0])
                maxQ = np.amax(self.target.predict(sprime)[0])
                prediction[action] = reward + self.gamma * maxQ

            print("Second prediction with correction: ", prediction)
        ...
        print("Batch of predictions: ", predictions)
